...
extern "C"
{
    ...
    namespace ESPTools
    {
        ...
        namespace BLE
        {
            static SemaphoreHandle_t scanReadySemphr = xSemaphoreCreateBinary();

            static constexpr char remote_device_name[] = "ESP_GATTS_DEMO";
            ...
            static void esp_gap_cb(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
            {
                CREATE_LOG_TAG("GAP CB");

                uint8_t *adv_name = nullptr;
                uint8_t adv_name_len = 0;
                static constexpr uint8_t MAX_ADV_NAME_BUFFER_LEN = 32;

                switch (event)
                {
                case ESP_GAP_BLE_SCAN_PARAM_SET_COMPLETE_EVT:
                {
                    xSemaphoreGive(scanReadySemphr);
                    break;
                }
                ...
                case ESP_GAP_BLE_SCAN_RESULT_EVT:
                {
                    ...
                    case ESP_GAP_SEARCH_INQ_RES_EVT:
                    {
                        ...
                        char advNameBuffer[MAX_ADV_NAME_BUFFER_LEN] = {0};
                        strncpy(advNameBuffer, (const char *)adv_name, adv_name_len);
                        ESP_LOGD(TAG,
                                 "MAC: %s | RSSI: %4d | Name: %-*s | Adv Data Len: %3d | Scan Response Len: %3d",
                                 MACtoStr(param->scan_rst.bda),
                                 param->scan_rst.rssi,
                                 MAX_ADV_NAME_BUFFER_LEN - 1, advNameBuffer,
            ...
            void initBLE()
            {
                ...
                if (ret)
                {
                    ESP_LOGE(TAG, "init bluetooth failed: %s\n", esp_err_to_name(ret));
                    return;
                }
                ...
                if (ret)
                {
                    ESP_LOGE(TAG, "enable bluetooth failed: %s\n", esp_err_to_name(ret));
                    return;
                }
            ...
            {
                static char MACtoStrBuffer[3 * ESP_BD_ADDR_LEN] = {0};
                sprintf(MACtoStrBuffer, "%02X:%02X:%02X:%02X:%02X:%02X", bda[0], bda[1], bda[2], bda[3], bda[4], bda[5]);
                return MACtoStrBuffer;
            }
            ...
            {
                CREATE_LOG_TAG("startScan");

                if (xSemaphoreTake(scanReadySemphr, portMAX_DELAY))
                    ESP_ERROR_CHECK(esp_ble_gap_start_scanning(duration));
                else
                    ESP_LOGE(TAG, "BLE start scan timeout");
            }
